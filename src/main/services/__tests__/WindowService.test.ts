...
describe("WindowService", () => {
    ...
    describe("服务初始化", () => {
        it("应该正确初始化服务", async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith("WindowService initialized with window management capabilities")
        })

        it("应该正确清理服务", async () => {
            await windowService.cleanup()
            expect(logger.info).toHaveBeenCalledWith("WindowService cleaning up")
        })
    ...
    describe("窗口创建", () => {
        it("应该创建新窗口", async () => {
            const result = await windowService.handleCreate({} as any, { width: 800, height: 600 }, "test-route")
    ...
    describe("窗口操作", () => {
        ...
        it("应该设置窗口边界", async () => {
            ...
            const result = await windowService.handleSetBounds({} as any, 12345, bounds)
        ...
        it("应该设置窗口标题", async () => {
            const result = await windowService.handleSetTitle({} as any, 12345, "New Title")
        ...
        it("应该设置窗口置顶", async () => {
            const result = await windowService.handleSetAlwaysOnTop({} as any, 12345, true)
        ...
        it("应该切换全屏模式", async () => {
            const result = await windowService.handleToggleFullscreen({} as any, 12345)
