...
import WorkerPool, { WorkerMessage, WorkerRequest, WorkerResponse } from "../core/WorkerPool"
...
describe("WorkerPool", () => {
    ...
    describe("Worker Type Registration", () => {
        it("should register a new worker type", () => {
            ...
            const mockWorkerFactory = jest.fn(() => new Worker("test") as jest.Mocked<Worker>)
        ...
        it("should set default values for optional config", () => {
            ...
            const mockFactory = jest.fn(() => new Worker("test") as jest.Mocked<Worker>)
            workerPool.registerWorkerType("SimpleWorker", config, mockFactory)

            // Should set defaults without throwing
            expect(() => {
                const mockFactory2 = jest.fn(() => new Worker("test") as jest.Mocked<Worker>)
    ...
    describe("Worker Execution", () => {
        beforeEach(() => {
            const execFactory = jest.fn(() => new Worker("test") as jest.Mocked<Worker>)
        ...
        it("should execute operation successfully", async () => {
            ...
            const executePromise = workerPool.execute("ExecutionWorker", "test-operation", { data: "test" })
        ...
        it("should handle worker errors", async () => {
            ...
            const executePromise = workerPool.execute("ExecutionWorker", "error-operation", { data: "test" })
        ...
        it("should retry failed operations", async () => {
            ...
            const result = await workerPool.execute("ExecutionWorker", "retry-operation", { data: "test" })

            expect(result).toBe("success after retry")
            expect(logger.info).toHaveBeenCalledWith(expect.stringContaining("Retrying operation retry-operation"))
        })
        ...
        it("should throw error for unregistered worker type", async () => {
            await expect(workerPool.execute("UnregisteredWorker", "test", {})).rejects.toThrow(
                "Worker type UnregisteredWorker not registered"
            )
        })
    })
    ...
    describe("Worker Pool Management", () => {
        beforeEach(() => {
            const poolFactory = jest.fn(() => new Worker("test") as jest.Mocked<Worker>)
            workerPool.registerWorkerType(
                "PoolWorker",
                {
                    maxWorkers: 2,
    ...
    describe("Worker Cleanup", () => {
        beforeEach(() => {
            const cleanupFactory = jest.fn(() => new Worker("test") as jest.Mocked<Worker>)
            workerPool.registerWorkerType(
                "CleanupWorker",
                {
                    maxWorkers: 1,
    ...
    describe("Worker Communication Protocol", () => {
        it("should send properly formatted request messages", async () => {
            mockWorkerInstance.on.mockImplementation(
                (event: string | symbol, handler: (...args: unknown[]) => void) => {
                    if (event === "message") {
                        ...
                        setTimeout(() => {
                            const sentMessage = mockWorkerInstance.postMessage.mock.calls[0][0] as WorkerRequest
            ...
            const result = await workerPool.execute("ExecutionWorker", "protocol-test", { data: "test" })
        ...
        it("should handle unknown message IDs gracefully", () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith(
                expect.stringContaining("Received message for unknown request unknown-id")
            )
        })
    })
})
