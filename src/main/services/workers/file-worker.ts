...
async function deleteFiles({ paths, permanent }: DeleteParams): Promise<any> {
    let deletedFiles = 0;
...
async function renameFile({ oldPath, newPath }: RenameParams): Promise<any> {
    try {
...
async function createFolder({ path: folderPath, recursive }: CreateFolderParams): Promise<any> {
    try {
...
async function getFileInfo({ path: filePath }: GetInfoParams): Promise<any> {
    try {
