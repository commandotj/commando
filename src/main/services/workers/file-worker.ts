...
async function deleteFiles({ paths, permanent }: DeleteParams): Promise<unknown> {
    let deletedFiles = 0
...
async function renameFile({ oldPath, newPath }: RenameParams): Promise<unknown> {
    try {
...
async function createFolder({ path: folderPath, recursive }: CreateFolderParams): Promise<unknown> {
    try {
...
async function getFileInfo({ path: filePath }: GetInfoParams): Promise<unknown> {
    try {
