...
export default class DirectoryService implements BaseService {
    ...
    async handleNavigate(
        ...
        try {
            ...
            // 获取目录内容
            const entries = await this.handleList(_event, { path: params.path });
    ...
    async handleCurrent(
        ...
        try {
            const entries = await this.handleList(event, { path: this.currentPath });
            return {
    ...
    async handleParent(
        ...
        if (parentPath === this.currentPath) {
            return {
                success: false,
                path: this.currentPath,
                entries: await this.handleList(_event, { path: this.currentPath })
            };
        }
