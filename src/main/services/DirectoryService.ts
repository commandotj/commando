...
import type { DirectoryEntry, ListDirectoryOptions } from "@common/types/DirectoryTypes"
...
export default class DirectoryService implements BaseService {
     ...
     * @returns 目录条目数组
     */
    async handleList(_event: IpcMainInvokeEvent, params: ListDirectoryParams): Promise<DirectoryEntry[]> {
        try {
            ...
            if (params.options?.sortBy) {
                this.sortDirectoryEntries(results, params.options.sortBy, params.options.sortOrder)
            }
     ...
     * @returns 当前目录路径和内容
     */
    async handleCurrent(event: IpcMainInvokeEvent): Promise<{ path: string; entries: DirectoryEntry[] }> {
        try {
     ...
     * @param params - 监控参数
     */
    async handleWatch(_event: IpcMainEvent, params: WatchDirectoryParams): Promise<void> {
        try {
            this.validateDirectoryPath(params.path)
