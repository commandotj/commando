...
import { WORKER_TYPE_REGISTERED, WORKER_CREATED, ORPHANED_WORKER_MESSAGE } from "../../../common/constants/LogMessages"
import {
...
class WorkerInstance extends EventEmitter {
    ...
    async execute<T = unknown>(
        ...
        if (!this.isIdle) {
            throw new Error(`Worker ${this.id} is busy with ${this.currentTask}`)
        }
...
export class WorkerPool {
    ...
    registerWorkerType(name: string, config: WorkerConfig, workerFactory: () => Worker): void {
    ...
    async execute<T = unknown>(
        ...
        try {
            ...
            if (retryCount < (config.maxRetries || 0)) {
                logger.info(`Retrying operation ${operation} (attempt ${retryCount + 1})`)
                return this.execute<T>(workerType, operation, payload, onProgress, retryCount + 1)
            }
    ...
    private async getOrCreateWorker(workerType: string): Promise<WorkerInstance> {
    ...
    private removeWorker(workerType: string, worker: WorkerInstance): void {
        ...
        if (pool) {
            ...
            if (index > -1) {
                pool.splice(index, 1)
                logger.info(`Removed worker ${worker.id} from pool ${workerType}`)
            }
        }
    }
    ...
    getPoolStats(): Record<string, { total: number; idle: number; busy: number }> {
    ...
    startIdleWorkerCleanup(): void {
        setInterval(() => {
            this.workerPools.forEach((pool, workerType) => {
                ...
                const idleWorkers = pool.filter(w => w.isIdle && w.idleTime > (config.idleTimeout || 300000))
                ...
                if (idleWorkers.length > 0) {
                    logger.info(`Cleaned up ${idleWorkers.length} idle workers for ${workerType}`)
                }
            })
