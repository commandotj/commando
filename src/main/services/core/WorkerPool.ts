...
class WorkerInstance extends EventEmitter {
    ...
    constructor(workerScript: string, workerData?: any) {
        ...
        if (!createWorker) {
            throw new Error(ErrorMessages.WORKER_SCRIPT_NOT_FOUND);
        }
...
export class WorkerPool {
    ...
    registerWorkerType(
        ...
        logger.info(LogMessages.WORKER_TYPE_REGISTERED, {
            workerType: name,
    ...
    async execute<T = unknown>(
        ...
        if (!config) {
            throw new Error(ErrorMessages.WORKER_TYPE_NOT_REGISTERED);
        }
    ...
    private async getOrCreateWorker(
        ...
        if (pool.length < config.maxWorkers) {
            ...
            pool.push(worker);
            logger.info(LogMessages.WORKER_CREATED, {
                workerType,
                workerId: worker.id,
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                reject(new Error(ErrorMessages.WORKER_TIMEOUT));
        ...
        setInterval(() => {
            this.workerPools.forEach((pool, workerType) => {
                ...
                const idleWorkers = pool.filter(
                    (w) =>
                        w.isIdle && w.idleTime > (config.idleTimeout || 300000)
                );
