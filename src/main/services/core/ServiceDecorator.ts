...
class ServiceRegistry {
     ...
     */
    register(serviceClass: new (mainWindow?: BrowserWindow) => BaseService): void {
    ...
    private bindIpcHandlers(service: BaseService, metadata: ServiceMetadata): void {
        metadata.ipcChannels.forEach((channelConfig: string | IpcChannelConfig) => {
            ...
            const config: IpcChannelConfig =
                typeof channelConfig === "string" ? { channel: channelConfig, type: "handle" } : channelConfig

            const methodName = config.methodName || this.channelToMethodName(config.channel)
            const method = (service as unknown as Record<string, (...args: unknown[]) => unknown>)[methodName]

            if (typeof method === "function") {
                if (config.type === "handle") {
                    // Bidirectional communication with invoke/handle
                    ipcMain.handle(config.channel, async (event: IpcMainInvokeEvent, ...args: unknown[]) => {
                        try {
                    ...
                    ipcMain.on(config.channel, async (event: IpcMainEvent, ...args: unknown[]) => {
                        try {
                ...
                logger.warn(`Method ${methodName} not found in service ${metadata.name}`)
            }
        })
    }
...
// Service decorator
export function Service(metadata: Omit<ServiceMetadata, "ipcChannels"> & { ipcChannels?: string[] }) {
        ...
        const EnhancedClass = class extends constructor {
            ...
            private autoDetectIpcChannels(): string[] {
                ...
                methodNames.forEach(methodName => {
                    if (methodName.startsWith("handle") && methodName !== "handleMessage") {
