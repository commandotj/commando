...
export function streamCopy({ source, destination, stats, options }: StreamCopyParams): Promise<{ bytesProcessed: number }> {
    return new Promise((resolve, reject) => {
        ...
        const readStream = gracefulFs.createReadStream(source, { highWaterMark: bufferSize })
        const writeStream = gracefulFs.createWriteStream(destination, { highWaterMark: bufferSize })
        ...
        const updateProgress = (): void => {
            if (options.progressCallback && totalBytes > 0) {
                ...
                const speed = timeSinceLastUpdate > 0 ? (bytesProcessed - lastBytesProcessed) / (timeSinceLastUpdate / 1000) : 0
