...
export interface PerformFileOperationDeps {
    ...
    streamCopy: (params: { source: string; destination: string; stats: gracefulFs.Stats; options: FileOperationOptions }) => Promise<{
...
export async function performFileOperation({ source, destination, options, deps }: PerformFileOperationParams): Promise<{
    bytesProcessed: number
    checksum?: string
}> {
    const { logger, ensureDirectoryExists, streamCopy, calculateChecksum, sleep, defaults } = deps
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            if (attempt > 0) {
                const delay = (options.retryDelay ?? defaults.retryDelay) * Math.pow(2, attempt - 1)
                if (options.verbose) {
                    logger.info(
            ...
            const copyResult = await streamCopy({ source, destination, stats: sourceStats, options })
            ...
            if (options.verifyIntegrity) {
                ...
                if (sourceChecksum !== destChecksum) {
                    throw new Error(`文件完整性验证失败: 源文件 ${sourceChecksum} != 目标文件 ${destChecksum}`)
                }
                checksum = destChecksum
            }

            if (options.preserveTimestamps) {
                await gracefulFs.promises.utimes(destination, sourceStats.atime, sourceStats.mtime)
            }
