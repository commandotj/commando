...
export class CommandExecutor {
    ...
    async executeCommand(command: string, options: CommandExecutionOptions = {}): Promise<CommandExecutionResult> {
        ...
        try {
            logger.info(`执行命令: ${command}`, { cwd: options.cwd, timeout: options.timeout })
    ...
    async executeCommandAsync(
    ...
    ): Promise<{ process: ChildProcess; promise: Promise<CommandExecutionResult> }> {
        const startTime = Date.now()
        ...
        return new Promise(resolve => {
            ...
            // 处理进程结束
            const promise = new Promise<CommandExecutionResult>(resolvePromise => {
                ...
                if (options.timeout) {
                    setTimeout(() => {
                        if (childProcess.pid && this.activeProcesses.has(childProcess.pid)) {
                            logger.warn(`命令执行超时，强制终止: ${command}`)
