...
export const fetchDirectory = createAsyncThunk(
    ...
    async ({ paneIndex, path }: { paneIndex: 0 | 1; path: string }, { dispatch }) => {
        // @ts-expect-ignore
...
const fileManagerSlice = createSlice({
    ...
    initialState,
    reducers: {
        setPanePath(state, action: PayloadAction<{ paneIndex: 0 | 1; path: string }>) {
            if (action.payload.path) {
                state.panes[action.payload.paneIndex].currentPath = action.payload.path;
            }
        },
        setPaneEntries(state, action: PayloadAction<{ paneIndex: 0 | 1; entries: FileEntry[] }>) {
