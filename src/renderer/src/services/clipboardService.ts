...
export class ClipboardService {
    ...
    async paste(targetPath: string, targetPane: 0 | 1): Promise<void> {
        ...
        if (!this.canPaste(targetPath)) {
            throw new Error('Cannot paste: clipboard is empty or target is invalid');
        }

        const operationType: FileOperationType = state.operation === 'cut' ? 'move' : 'copy';
        const operationId = `${operationType}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

        // Start the file operation
        store.dispatch(startOperation({
            id: operationId,
            type: operationType,
            source: state.items,
        ...
        try {
            ...
            if (operationType === 'copy') {
                ...
                throw new Error('Move operations not yet implemented in file system API');
            }
