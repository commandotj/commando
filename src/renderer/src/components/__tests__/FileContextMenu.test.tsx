...
jest.mock("@radix-ui/react-context-menu", () => ({
    ...
    Trigger: ({ children }: { children: React.ReactNode }) => <div data-testid="context-menu-trigger">{children}</div>,
    Portal: ({ children }: { children: React.ReactNode }) => <div data-testid="context-menu-portal">{children}</div>,
    Content: ({ children }: { children: React.ReactNode }) => <div data-testid="context-menu-content">{children}</div>,
    Item: ({
        children,
        onClick,
        disabled,
        ...
        <div data-testid="context-menu-item" onClick={onClick} data-disabled={disabled}>
            {children}
        </div>
    ),
...
const createTestStore = (initialState = {}): ReturnType<typeof configureStore> => {
...
const renderWithStore = (component: React.ReactElement, initialState = {}): ReturnType<typeof render> => {
...
describe("FileContextMenu", () => {
    ...
    it("should show copy and cut options when files are selected", () => {
        renderWithStore(
            <FileContextMenu selectedFiles={["/test/file1.txt", "/test/file2.txt"]} currentPath="/test" currentPane={0}>
                <div data-testid="trigger">Right click me</div>
            </FileContextMenu>
        )
    ...
    it("should show rename option for single file selection", () => {
        renderWithStore(
            <FileContextMenu selectedFiles={["/test/file.txt"]} currentPath="/test" currentPane={0}>
                <div data-testid="trigger">Right click me</div>
            </FileContextMenu>
        )
    ...
    it("should disable copy to other pane when other pane has no path", () => {
        ...
        expect(copyToOtherPane.closest('[data-disabled="true"]')).toBeInTheDocument()
    })
})
