...
export function ResizableTable<T extends object>({
    ...
    const allSelected =
        dataSource.length > 0 &&
        dataSource.every((row, idx) => selectedRowKeys.includes(getRowKey(row, idx)));
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            onRowSelectionChange?.(dataSource.map((row, idx) => getRowKey(row, idx)));
        } else {
            onRowSelectionChange?.([]);
        }
    };
                    ...
                    {table.getRowModel().rows.map((row, idx) => (
                        <div
                            ...
                            className={
                                ...
                                (selectedRowKeys.includes(getRowKey(dataSource[idx], idx))
