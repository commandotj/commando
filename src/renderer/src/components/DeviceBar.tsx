...
export const DeviceBar: React.FC<DeviceBarProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex flex-row gap-2 px-4 py-1 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 items-center overflow-x-hidden min-h-[2.5rem]"
    ...
    >
      ...
      {loading && (
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          ...
          <span>{loadingMessage || (t("ui.drives.scanning") as string)}</span>
        </div>
      )}
                ...
                >
                  {overflow.map((dev) => (
                    <DropdownMenu.Item
                      ...
                      onSelect={() => onDeviceClick(dev.mountPath)}
                      title={dev.description || dev.device}
                    >
                      {dev.mountPath === "/" ? (
