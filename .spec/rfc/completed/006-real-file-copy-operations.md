...
本 RFC 提出了一个完整的文件操作系统，**100% 复制 Total Commander 的核心文件操作功能**，包括复制、移动、删除、重命名等所有操作，同时采用**现代化 UI 设计风格**与项目整体设计保持一致。基于RFC-2025-010时空人格化架构，通过鲁班引擎（LubanEngine）实现专业级文件操作，由孔子引擎协调IPC通信，确保刘邦（渲染进程）与春秋战国（主进程）的完美协作。目标是提供专业级的文件管理体验，同时保持现代化的用户界面。
...
基于RFC-2025-010时空人格化架构理念，我们决定采用 **Total Commander 完整功能集** + **现代化 UI 设计** + **graceful-fs 精确操作引擎**的混合方案。这个方案在专业功能、现代设计和技术实现之间达到最佳平衡，完美体现了春秋战国诸子百家的协作精神。

#### 时空架构集成

- **刘邦（渲染进程）**：统一管理用户界面，委托韩信发送IPC请求
- **萧何（Store管理）**：管理文件操作状态，协调组件间数据流
- **韩信（Preload）**：负责IPC通信，连接汉朝与春秋战国
- **司马迁（渲染进程日志）**：**汉朝史学家**，负责记录汉朝历史，通过时空隧道发送到春秋战国
- **孔子（KongziEngineManager）**：**春秋战国时空通道协调者，唯一可以接收IPC请求并发送响应的角色**，协调各引擎协作
- **鲁班（LubanEngine）**：**春秋战国工匠大师**，执行专业级文件操作，工匠精神体现
- **左丘明（Git版本控制）**：**春秋史学家**，负责Git版本控制和代码历史记录
- **老子（主进程日志）**：**道家创始人**，负责记录春秋战国历史，与司马迁形成史学双雄

#### 重要架构边界调整

- **孔子是唯一的IPC入口点**：所有来自汉朝的请求都必须通过孔子接收
- **孔子统一协调**：孔子接收请求后，委托合适的引擎（鲁班、墨子等）执行
- **时空隧道管理**：孔子负责所有跨时空通信的协调和管理
- **服务层边界**：其他服务（DirectoryService、DriveService等）不再直接处理IPC，而是通过孔子委托
...
5. **时空协作**: 遵循RFC-2025-010架构理念，实现汉朝与春秋战国的完美协作
...
class LubanEngine {
    ...
    async copyWithIntegrity(src: string, dest: string): Promise<void> {
        return this.concurrencyLimit.add(async () => {
            ...
            // 4. 完整性验证
            const destChecksum =
                await this.checksumValidator.calculate(tempDest);
...
#### 3.1 Total Commander 复制操作流程（完全复制TC体验）

1. **触发方式**：
    - **右键菜单**：选择文件 → 右键 → "Copy to Other Pane"
    - **快捷键**：F5 键复制到另一侧
    - **按钮操作**：FunctionBar 复制按钮

2. **队列系统**：
    - **非阻塞模态**：复制进度对话框不阻塞主界面操作
    - **任务队列**：新选择的文件自动加入队列，根据并行任务设置决定是否立即开始
    - **队列管理**：支持多个复制任务排队执行

3. **双进度条设计**（完全复制Total Commander）：
    - **整体进度条**：显示所有文件的总体复制进度
    - **当前文件进度条**：显示当前正在复制文件的进度
    - **实时信息**：当前文件名、复制速度、剩余时间、已复制/总大小

4. **进度展示界面**：
    - **Header区域**：操作类型 + 源路径 → 目标路径
    - **双进度区域**：
        - 主进度条：整体百分比 + 文件计数（如 "3/10 files"）
        - 当前文件进度条：当前文件百分比 + 文件名
    - **统计信息**：复制速度、剩余时间、已处理字节数
    - **操作按钮**：Background（后台）、Pause（暂停）、Cancel（取消）

5. **队列行为**：
    - **新任务加入**：选择新文件时自动加入队列
    - **并行控制**：根据系统设置决定是否并行执行
    - **任务切换**：可在队列中切换不同任务查看进度
...
#### 3.3 Total Commander 功能完全对比表

| 功能特性         | Total Commander           | Commando React 实现       | 状态      |
| ---------------- | ------------------------- | ------------------------- | --------- |
| **右键菜单复制** | 右键 → Copy to Other Pane | ✅ 已实现                 | ✅ 完成   |
| **双进度条**     | 整体进度 + 当前文件进度   | ✅ BatchCopyProgressModal | ✅ 完成   |
| **非阻塞模态**   | 可继续操作其他文件        | ✅ 非阻塞Dialog           | ✅ 完成   |
| **任务队列**     | 多任务排队执行            | ✅ WorkerPool队列系统     | ✅ 完成   |
| **实时统计**     | 速度、剩余时间、文件数    | ✅ 实时更新               | ✅ 完成   |
| **后台执行**     | Background按钮            | ✅ Background功能         | ✅ 完成   |
| **暂停/取消**    | Pause/Cancel按钮          | ✅ 暂停取消功能           | ✅ 完成   |
| **冲突处理**     | 覆盖/跳过/重命名选项      | 🔄 待完善                 | 🚧 进行中 |
| **批量操作**     | 多文件同时复制            | ✅ 批量复制支持           | ✅ 完成   |
| **进度精度**     | 字节级精度                | ✅ 字节级跟踪             | ✅ 完成   |
...
interface TaskQueue {
    // 当前执行中的任务
    activeTasks: CopyTask[];

    // 等待队列
    pendingTasks: CopyTask[];
...
1. **新任务加入**：选择文件后自动创建CopyTask并加入队列
2. **并行控制**：根据maxConcurrency设置决定同时执行的任务数
3. **任务切换**：用户可在队列中查看不同任务的进度
4. **优先级处理**：高优先级任务（如用户直接操作）优先执行
...
基于RFC-2025-010时空人格化架构，定义从汉朝到春秋战国的完整协作流程。

#### 2.1 时空协作流程（完全符合RFC-2025-010）

**完整流程**：刘邦 → 萧何 → 韩信 → 鲁定公 → 孔子 → 鲁班

1. **刘邦（渲染进程）**：用户选择文件，统一管理界面
2. **萧何（Store管理）**：管理文件操作状态，协调组件间数据流
3. **韩信（Preload）**：通过时空隧道发送IPC请求到春秋战国
4. **鲁定公（LuDinggongService）**：**IPC桥梁**，接收IPC请求并委托孔子协调
5. **孔子（KongziEngineManager）**：**引擎协调者**，统一管理春秋战国诸子百家
6. **鲁班（LubanEngine）**：执行专业级文件操作，工匠精神体现

**架构分层原则**：

- **Service层（鲁定公）**：IPC桥梁，不包含业务逻辑，纯粹的通信层
- **Engine层（孔子/鲁班）**：纯粹的业务逻辑，不依赖Electron IPC，可独立测试

#### 2.2 时空隧道通信契约

- **汉朝入口**: `window.fsApi.copyBatch(sources, destination)`
- **时空隧道**: 韩信通过 `ipcRenderer.invoke("copy:batch", params)` 发送请求
- **春秋战国接收**: 鲁定公（LuDinggongService）**接收IPC请求**（Service层）
- **委托协调**: 鲁定公委托孔子（KongziEngineManager）协调引擎（Engine层）
- **孔子协调**: 孔子接收任务后，委托鲁班执行文件操作
- **鲁班执行**: 鲁班（LubanEngine）执行专业级文件复制
- **时空隧道返回**: 鲁定公通过时空隧道返回结果给韩信
- **汉朝接收**: 韩信接收结果，萧何更新状态，刘邦更新UI
        ...
        if (this.copiedBytes - this.lastUpdateBytes >= this.updateThreshold) {
            ...
            this.onProgress({
                ...
                percentage: Math.round(
                    (this.copiedBytes / this.totalSize) * 100
                ),
...
### Phase 1: 时空架构代码对齐 (Week 1-2)

- [ ] **时空人格化角色体现**
    - [ ] 在 `App.tsx` 中添加刘邦角色注释（汉朝皇帝，统一管理）
    - [ ] 在 `store.ts` 中添加萧何角色注释（汉朝丞相，Store管理）
    - [ ] 在 `preload/index.ts` 中添加韩信角色注释（汉朝将军，IPC通信）
    - [ ] 在 `ipc.ts` 中添加孔子角色注释（时空通道协调者）
    - [ ] 在 `LubanEngine.ts` 中添加鲁班角色注释（工匠大师，文件操作）

- [x] **时空隧道通信统一**（已完成）
    - [x] 在 `preload/index.ts` 中添加"时空隧道"概念注释
    - [x] 在 `ipc.ts` 中统一IPC通信模式说明（已迁移到孔子引擎）
    - [x] 在 `ServiceDecorator.ts` 中体现时空通道管理
    - [x] 在 `KongziEngineManager.ts` 中体现孔子协调作用
    - [x] **重要调整**：孔子是唯一可以接收IPC请求并发送响应的角色

- [ ] **诸子百家协作体现**
    - [ ] 在 `CopyService.ts` 中体现孔子委托鲁班的协作
    - [ ] 在 `WorkerPool.ts` 中体现春秋战国诸子百家精神
    - [ ] 在 `BatchCopyProgressModal.tsx` 中体现汉朝与春秋战国的协作
    - [ ] 在关键流程中添加"刘邦委托韩信→孔子协调→鲁班执行"的注释
...
### Phase 2: Total Commander 核心功能实现 (Week 3-4)

- [x] **右键菜单复制功能**
    - [x] 右键菜单 "Copy to Other Pane" 选项
    - [x] 快捷键 F5 复制到另一侧
    - [x] FunctionBar 复制按钮
