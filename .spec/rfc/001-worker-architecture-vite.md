...
export class CopyService implements BaseService {
        ...
        const taskHandle = await this.workerPool.submitTask({
            ...
            callbacks: {
                onProgress: (progress, context) =>
                    this.routeProgressToUI(progress, context),
                onComplete: (result, context) =>
                    this.routeCompletionToUI(result, context),
                onError: (error, context) =>
                    this.routeErrorToUI(error, context),
            },
        });
    ...
    private routeProgressToUI(
        ...
        if (requestContext && requestContext.ipcEvent) {
            ...
            requestContext.ipcEvent.sender.send(
                IPCChannels.FILE_OPERATIONS.COPY.PROGRESS,
                {
...
_详细常量定义参见RFC-2025-002（常量管理系统）_
    ...
    public routeWorkerMessage(message: WorkerProtocol.Message): void {
        ...
        switch (message.messageType) {
            case MessageType.PROGRESS_REPORT:
                this.routeProgressToUI(
                    message as ProgressReport,
                    requestContext
                );
                break;
            case MessageType.TASK_RESPONSE:
                this.routeCompletionToUI(
                    message as TaskResponse,
                    requestContext
                );
                break;
            default:
...
export class WorkerErrorHandler {
    ...
    private notifyTaskFailure(task: ActiveTask, failure: TaskFailure): void {
        ...
        if (requestContext) {
            requestContext.ipcEvent.sender.send(
                IPCChannels.FILE_OPERATIONS.COPY.ERROR,
                {
                    requestId: task.requestId,
                    taskId: task.taskId,
                    error: failure,
