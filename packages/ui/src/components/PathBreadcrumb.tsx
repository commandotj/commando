...
const PathBreadcrumb: React.FC<PathBreadcrumbProps> = ({
    ...
    const renderHiddenSegments = (): JSX.Element => (
        <div
            style={{
                position: "absolute",
                visibility: "hidden",
        ...
        >
            {parts.map((part, idx) => (
                <a
                    ...
                    className="cmd-breadcrumb__segment-measure"
                >
                    {part}
                </a>
            ))}
    ...
    const renderBreadcrumb = (): JSX.Element => {
        ...
        for (let i = 0; i < parts.length; i++) {
            if (visibleIdx.includes(i)) {
                if (items.length > 0) {
                    items.push(
                        <span key={`sep-${i}`} className="cmd-breadcrumb__sep">
                            /
                        </span>
                    );
                }
                items.push(
                    <a
                        ...
                        className="cmd-breadcrumb__link"
                        title={parts[i]}
                    >
                        {parts[i]}
                    </a>
                ...
                // 插入 ...
                items.push(
                    <span key="ellipsis-sep" className="cmd-breadcrumb__sep">
                        /
                    </span>
                );
                items.push(
                    <div
                        key="ellipsis"
                        className="cmd-breadcrumb__ellipsis-wrap"
                        ref={menuRef}
                    >
                        <button
                            type="button"
                            className="cmd-breadcrumb__ellipsis-btn"
                        ...
                        {menuOpen && (
                            <div className="cmd-breadcrumb__menu">
                                {foldedIdx.map(idx => (
                                    <div
                                        key={idx}
                                        className="cmd-breadcrumb__menu-item"
                                        title={parts[idx]}
                                        onClick={e => {
                                            e.preventDefault();
                                            setMenuOpen(false);
    ...
    return (
        <nav
            className={`cmd-breadcrumb ${className}`}
        ...
        >
            ...
            <ol className="cmd-breadcrumb__list">{renderBreadcrumb()}</ol>
        </nav>
    );
};
