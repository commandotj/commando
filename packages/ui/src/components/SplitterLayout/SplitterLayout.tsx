...
function clearSelection(): void {
    if (
        ...
        if (
            selection &&
            (selection as unknown as { empty?: () => void }).empty
        ) {
            (selection as unknown as { empty: () => void }).empty();
        ...
        (
            document as unknown as { selection: { empty: () => void } }
        ).selection.empty();
    }
}
...
> {
    ...
    componentDidUpdate(
        _prevProps: SplitterLayoutProps,
    ...
    handleSplitterMouseDown(e: React.MouseEvent | React.TouchEvent): void {
        e.preventDefault();
        clearSelection();
        ...
        return (
            ...
            >
                ...
                {wrappedChildren.length > 1 && (
                    <div
                        role="separator"
                        className="layout-splitter wails-no-drag"
                        ref={c => {
                            this.splitter = c;
                        }}
