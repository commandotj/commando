# CLAUDE.md - AI Assistant Guide for Commando React
...
- **Architecture**: Electron + React + TypeScript + Redux Toolkit
- **Build System**: electron-vite with externalizeDepsPlugin
- **Testing**: Jest for main process, React Testing Library for renderer
- **Styling**: Chakra UI + Radix UI + Tailwind CSS
...
npm run typecheck       # Type checking (both node and web)
...
The project uses a custom service architecture with:

- **@Service decorator** for auto-registration (src/main/services/core/ServiceDecorator.ts)
- **WorkerPool** for shared background processing (src/main/services/core/WorkerPool.ts)
- **Automatic IPC binding** based on service method names
- **TypeScript worker threads** for CPU-intensive operations

### Directory Structure

```
src/
├── common/                  # Shared across renderer, main, preload
│   ├── constants/          # IPC channels, message types, operation types
│   ├── types/             # TypeScript interfaces and types
│   └── utils/             # Utility functions
├── shared/                  # Shared between main and preload only
│   ├── ipc/               # IPC channel definitions and handlers
│   └── protocols/         # Communication protocols
├── main/                    # Electron main process
│   ├── services/           # Service-oriented file operations
│   │   ├── core/          # Core service infrastructure
│   │   ├── workers/       # Background worker implementations
│   │   └── __tests__/     # Jest tests for services
...
- **NO HARDCODED STRINGS**: All IPC channels must be defined in src/common/constants/
...
- **Monitoring**: Complete metrics collection and performance analysis
...
- **Configuration**: `jest.config.js` in root
- **Mocks**: Pre-configured for Electron, fs, worker_threads
- **Run**: `npm test`
...
const result = await service.handleOperation(mockEvent, params);
expect(result).toEqual(expectedResult);
```

### Renderer Process (React Testing Library)

- **Location**: `src/renderer/src/components/__tests__/`
...
- Main process runs in Node.js environment (not browser)
- `externalizeDepsPlugin` handles Node.js dependencies
- Workers must use Node.js modules (`node:fs`, `node:worker_threads`)
...
- **Enterprise-grade patterns**: Use proper interfaces, factories, managers, and observers
- **Production-ready code**: Include diagnostics, metrics, configuration, and extensibility
- **Separation of concerns**: Clear responsibilities between layers and components
- **Comprehensive type systems**: Full TypeScript coverage with proper generics and constraints
- **No hardcoded strings**: All constants must be properly defined and categorized
- **Proper directory structure**: Use src/common/ for renderer-main-preload sharing, src/shared/ for main-preload only

### 4. NO HARDCODED STRINGS RULE (CRITICAL)

**ABSOLUTE PROHIBITION**: Never use hardcoded strings in code. ALL strings must be defined as constants in the appropriate namespace.

**Constant Management Reference**: See RFC-2025-002 (Constant Management System) for complete specifications
...
- **ZERO TOLERANCE**: Any PR with hardcoded strings will be rejected
- Use TypeScript's `as const` assertion for all constant definitions
...
private validateParams(params: MyParams): void {
    if (!params.required) {
        throw new Error(ErrorMessages.INVALID_PARAMS_REQUIRED_MISSING);
    }
}
```
...
### 1. Adding New File Operations

1. **Design phase**: Create or update RFC if significant
2. **Service implementation**: Create service class with @Service decorator
3. **Worker implementation**: Only if CPU-intensive (see RFC-2025-001)
4. **Testing**: Comprehensive Jest tests
5. **Integration**: Test with renderer components
6. **Documentation**: Update relevant RFCs
...
### 3. File Manager UI Integration

- **FilePane.tsx**: Main panel component
- **ResizableTable.tsx**: File list display
- **Redux slices**: State management in src/renderer/src/app/
- **IPC calls**: Use electron's ipcRenderer.invoke()
...
- **Author**: albert.li (albert_lee@hotmail.com)
- **Homepage**: https://www.systembug.com/commando
- **Repository**: Electron + React dual-panel file manager
- **Current Branch**: Usually feature branches, main branch for PRs

## Quick Start for New Contributors

1. **Clone and install**: `npm install`
2. **Read RFCs**: Start with docs/rfc/README.md
3. **Run tests**: `npm test` (should pass 100%)
4. **Start dev**: `npm run dev`
5. **Understand architecture**: Review service pattern in src/main/services/core/
6. **Make changes**: Follow service pattern, add tests, update RFCs
7. **Quality check**: typecheck + lint + test + build must all pass
