...
export class MoziEngine implements BaseEngine {
    ...
    private async performSync(
        ...
        // 分析同步项目
        const items = await this.analyzeSyncItems(
            sourcePath,
            targetPath,
            config
        );
            ...
            try {
                ...
                if (progressCallback) {
                    progressCallback({
                        taskId: "",
                        percentage:
                            ((processedItems + skippedItems + erroredItems) /
                                items.length) *
                            100,
                ...
                this.deps.logger.warn(`同步失败: ${item.relativePath}`, {
                    error,
                });
            }
        }
    ...
    private async scanDirectory(
        ...
        try {
            const entries = await this.deps.fs.readdir(currentPath, {
                withFileTypes: true,
            });
            ...
            for (const entry of entries) {
                ...
                // 检查排除模式
                if (
                    this.shouldExclude(itemRelativePath, config.excludePatterns)
                ) {
                    continue;
                }
    ...
    private async scanForExtraneous(
        ...
        try {
            const entries = await this.deps.fs.readdir(targetPath, {
                withFileTypes: true,
            });
            ...
            for (const entry of entries) {
                ...
                const correspondingSourcePath = path.join(
                    sourceBasePath,
                    itemRelativePath
                );
    ...
    private async syncItem(
        item: SyncItem,
        config: SyncTaskConfig
    ): Promise<{ synced: boolean; bytesTransferred: number }> {
        try {
            // 确保目标目录存在
            ...
            this.deps.logger.warn(`同步文件失败: ${item.relativePath}`, {
                error,
            });
            throw error;
        }
