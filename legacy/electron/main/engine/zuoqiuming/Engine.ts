...
export class ZuoqiumingEngine implements BaseEngine {
    ...
    constructor(
        deps: ZuoqiumingEngineDeps = { logger, fs: gracefulFs.promises }
    ) {
        this.deps = deps;
    }
        ...
        try {
            ...
            if (excludePatterns && excludePatterns.length > 0) {
                for (const pattern of excludePatterns) {
                    try {
                        await execAsync(`git reset HEAD -- "${pattern}"`, {
                            cwd: repoPath,
                        });
                    } catch {
                        // 忽略不存在的文件
    ...
    private async createCommit(
        ...
        try {
            // 设置作者信息
            if (author) {
                await execAsync(`git config user.name "${author.name}"`, {
                    cwd: repoPath,
                });
                await execAsync(`git config user.email "${author.email}"`, {
                    cwd: repoPath,
                });
            }
