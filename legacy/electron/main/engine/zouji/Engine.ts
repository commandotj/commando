...
export class ZoujiEngine implements BaseEngine {
    ...
    private async performComparison(
        ...
        try {
            ...
            if (
                config.compareContent !== false &&
                stats1.isFile() &&
                stats2.isFile()
            ) {
                contentComparison = await this.compareContent(
                    file1,
                    file2,
                    config
                );
            ...
            this.deps.logger.warn(`文件比较失败: ${file1} vs ${file2}`, {
                error,
            });
            throw error;
        }
    ...
    private compareAttributes(
        stats1: gracefulFs.Stats,
        stats2: gracefulFs.Stats
    ): AttributeComparison {
    ...
    private async compareContent(
        ...
        if (
            config.maxFileSize &&
            (stats1.size > config.maxFileSize ||
                stats2.size > config.maxFileSize)
        ) {
            return {
                identical: false,
    ...
    private async compareTextContent(
        ...
        return {
            ...
            method: "text",
            similarity,
            textDifferences: identical
                ? []
                : await this.generateTextDiff(text1, text2),
        };
    }
     ...
     * 比较二进制内容
     */
    private async compareBinaryContent(
        file1: string,
        file2: string
    ): Promise<ContentComparison> {
        ...
        const similarity = identical
            ? 1.0
            : this.calculateBinarySimilarity(buffer1, buffer2);
     ...
     * 计算二进制相似度
     */
    private calculateBinarySimilarity(
        buffer1: Buffer,
        buffer2: Buffer
    ): number {
    ...
    private generateComparisonResult(
        ...
        if (contentComparison && !contentComparison.identical) {
            differences.push({
                ...
                severity:
                    contentComparison.similarity > 0.8
                        ? "low"
                        : contentComparison.similarity > 0.5
                          ? "medium"
                          : "high",
            });
        }
        ...
        const similarity =
            contentComparison?.similarity || (identical ? 1.0 : 0.0);
    ...
    private createMissingFileResult(
        missingFile: string,
        existingFile: string,
        startTime: number
    ): ComparisonResult {
    ...
    private determineDifferenceType(
        ...
        if (
            differences.some(d => d.type === DifferenceType.CONTENT_DIFFERENT)
        ) {
