...
class ServiceRegistry {
    ...
    register(
        ...
        logger.info(
            `Registering service: ${metadata.name} v${metadata.version}`
        );
    ...
    private bindIpcHandlers(
        ...
        metadata.ipcChannels.forEach(
            (channelConfig: string | IpcChannelConfig) => {
                ...
                const methodName =
                    config.methodName ||
                    this.channelToMethodName(config.channel);
                const method = (
    ...
    async initializeAll(): Promise<void> {
        logger.info(
            `Initializing all services (${this.services.size} services)`
        );
        ...
        const initPromises = Array.from(this.services.values()).map(
            async service => {
                try {
                    ...
                    logger.info(
                        `Service ${service.getMetadata().name} initialized`
                    );
                } catch (error) {
                    logger.error(
    ...
    return function <T extends new (...args: any[]) => any>(constructor: T): T {
        ...
        const EnhancedClass = class extends constructor {
            ...
            constructor(...args: any[]) {
                ...
                const channels =
                    metadata.ipcChannels || this.autoDetectIpcChannels();
