...
export default class DriveService implements BaseService {
    ...
    async handleList(
        ...
        try {
            logger.info(
                `开始获取磁盘列表，选项: ${JSON.stringify(params.options)}`
            );
            ...
            this.sendLoadingState(
                false,
                error instanceof Error ? error.message : "获取磁盘列表失败",
                true
            );
    ...
    async handleRefresh(): Promise<DriveInfo[]> {
        try {
            ...
            this.sendLoadingState(
                false,
                error instanceof Error ? error.message : "刷新磁盘列表失败",
                true
            );
    ...
    private async refreshDriveCache(): Promise<void> {
        try {
            ...
            // 记录获取时间用于性能监控
            logger.debug(
                `获取驱动器信息耗时: ${fetchTime}ms，找到 ${fsSize.length} 个文件系统`
            );
            ...
            this.driveCache = fsSize
                ...
                .map((fs: Systeminformation.FsSizeData) => {
                    ...
                    const blockDevice = blockDevices.find(
                        (device: Systeminformation.BlockDevicesData) =>
                            device.name === fs.fs
                    );
            ...
            logger.debug(
                `磁盘缓存已刷新，共 ${this.driveCache.length} 个驱动器`
            );
        } catch (error) {
