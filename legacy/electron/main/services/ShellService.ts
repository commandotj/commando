...
import { LubanEngine } from "../engine/luban/Engine";
...
export default class ShellService implements BaseService {
    ...
    getMetadata(): ServiceMetadata {
        return {
            ...
            description:
                "Shell command execution and process management service",
        };
    }
    ...
    async handleExecuteAsync(
        ...
        const { process, promise } =
            await this.commandExecutor.executeCommandAsync(
                command,
                options,
                data => {
                    // 发送标准输出
    ...
    async handleFileOperation(
        ...
        SecurityValidator.logSecurityEvent(
            "file-operation",
            `${params.operation}:${params.source}`,
            securityResult
        );
        ...
        try {
            switch (params.operation) {
                case "copy":
                    ...
                    result = await this.lubanEngine.copyFile(
                        params.source,
                        params.destination,
                        {
                            overwrite: params.options?.overwrite,
                            preserveTimestamps:
                                params.options?.preserveTimestamps,
                ...
                case "move":
                    ...
                    result = await this.lubanEngine.moveFile(
                        params.source,
                        params.destination,
                        {
                            overwrite: params.options?.overwrite,
                ...
                case "create":
                    ...
                    result = await this.lubanEngine.getFileInfo(
                        params.destination
                    );
                    break;
