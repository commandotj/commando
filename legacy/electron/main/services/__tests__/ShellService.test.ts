...
describe("ShellService", () => {
    ...
    describe("命令执行", () => {
        it("应该执行同步命令", async () => {
            ...
            const result = await shellService.handleExecute(
                {} as any,
                "echo test"
            );
        ...
        it("应该执行异步命令", async () => {
            ...
            const result = await shellService.handleExecuteAsync(
                {} as any,
                "echo async test"
            );

            expect(
                mockCommandExecutor.executeCommandAsync
    ...
    describe("文件操作", () => {
        it("应该执行复制操作", async () => {
            ...
            expect(
                mockSecurityValidator.validateFileOperation
            ).toHaveBeenCalledWith("copy", "/source", "/dest");
            expect(mockLubanEngine.copyFile).toHaveBeenCalledWith(
                "/source",
                "/dest",
                {
                    overwrite: true,
        ...
        it("应该执行移动操作", async () => {
            ...
            expect(mockLubanEngine.moveFile).toHaveBeenCalledWith(
                "/source",
                "/dest",
                {
                    overwrite: undefined,
    ...
    describe("进程管理", () => {
        ...
        it("应该获取进程信息", async () => {
            ...
            const result = await shellService.handleProcessInfo(
                {} as any,
                1234
            );

            expect(mockProcessManager.getProcessInfo).toHaveBeenCalledWith(
                1234
            );
        ...
        it("应该终止进程", async () => {
            ...
            const result = await shellService.handleProcessKill(
                {} as any,
                1234,
                "SIGTERM"
            );

            expect(mockProcessManager.killProcess).toHaveBeenCalledWith(
                1234,
                "SIGTERM"
            );
    ...
    describe("安全验证", () => {
        it("应该验证命令安全性", async () => {
            ...
            mockSecurityValidator.validateCommand.mockReturnValue(
                mockValidation
            );

            const result = await shellService.handleValidateCommand(
                {} as any,
                "echo test"
            );
        ...
        it("应该验证路径安全性", async () => {
            ...
            const result = await shellService.handleValidatePath(
                {} as any,
                "/tmp/test"
            );

            expect(mockSecurityValidator.validatePath).toHaveBeenCalledWith(
                "/tmp/test"
            );
            expect(result).toEqual(mockValidation);
        });
    ...
    describe("系统资源", () => {
        it("应该获取系统资源信息", async () => {
            ...
            mockProcessManager.getSystemResources.mockResolvedValue(
                mockResources
            );
    ...
    describe("错误处理", () => {
        ...
        it("应该处理命令执行错误", async () => {
            mockCommandExecutor.executeCommand.mockRejectedValue(
                new Error("命令执行失败")
            );

            await expect(
                shellService.handleExecute({} as any, "invalid-command")
