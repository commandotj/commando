...
export interface MockReadStream {
    pipe: jest.MockedFunction<
        (destination: MockWriteStream) => MockWriteStream
    >;
...
export interface MockWriteStream {
    on: jest.MockedFunction<
        (event: string, callback: () => void) => MockWriteStream
    >;
    destroy: jest.MockedFunction<() => void>;
}
...
export interface MockHash {
    ...
    digest: jest.MockedFunction<{
        (): Buffer;
        (encoding: crypto.BinaryToTextEncoding): string;
    }>;
