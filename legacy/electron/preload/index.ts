...
import type { CopyWorkerMessage } from "../types/copy";
import "./logger";
...
contextBridge.exposeInMainWorld("fsApi", {
     ...
     * 监听复制状态更新事件 - 孔子主动报告状态
     */
    onCopyStatusUpdate: (cb: (status: any) => void) => {
        ipcRenderer.on("copy-status-update", (_event, status) => cb(status));
    },
    /**
     * 查询复制队列状态
     */
     ...
     * 查询复制任务状态
     */
    getCopyStatus: (taskId: string) =>
        ipcRenderer.invoke("copy:status", { taskId }),
    ...
    confirmCopyAction: (
        ...
        ipcRenderer.invoke("copy:confirm", {
            taskId,
            action,
            newName,
            applyToAll,
